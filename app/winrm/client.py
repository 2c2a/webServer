...
class AsyncWinRMClient:
    ...
    async def from_host_config(cls, host) -> "AsyncWinRMClient":
        ...
        password: Optional[str] = None
        encrypted = getattr(host, "_password", None)
        if encrypted:
            try:
                ...
                logger.warning("field_cipher 解密密码失败，尝试 password 属性: %s", e)
                password = None
        if not password:
    ...
    async def _send_command(
        ...
        """发送命令，返回 CommandId。"""
        args_xml = "".join(
            f"<rsp:Arguments>{xml_escape(a)}</rsp:Arguments>" for a in arguments
        )
        # MS-WSMV 规范：外层 rsp:Command 包裹命令行与参数
        body = (
            "<rsp:Command>"
    ...
    async def _receive_output(
        ...
        for _ in range(max_iterations):
            body = (
                "<rsp:Receive>"
                f"<rsp:CommandId>{xml_escape(command_id)}</rsp:CommandId>"
                "<rsp:DesiredStream>stdout</rsp:DesiredStream>"
                "<rsp:DesiredStream>stderr</rsp:DesiredStream>"
                "</rsp:Receive>"
            )
            envelope = self._transport._build_envelope(
    ...
    async def _signal_command(
        ...
        body = (
            "<rsp:Signal>"
            f"<rsp:CommandId>{xml_escape(command_id)}</rsp:CommandId>"
            f"<rsp:SignalCode>{SIGNAL_TERMINATE}</rsp:SignalCode>"
            "</rsp:Signal>"
        )
        envelope = self._transport._build_envelope(
