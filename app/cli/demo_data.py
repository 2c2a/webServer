...
在 demo 模式下，每次启动自动清理 demo 站点组下的所有业务数据并重建一份新的，
让后台列表页（主机/产品/工单/积分/公告等）有可视化内容。

设计要点：
- 清理范围：demo 站点组下的所有业务数据（含用户演示期间手动创建的），
  保留演示账号（superadmin/siteadmin/user）和 demo 站点组本身。
- 清理依据：按 site_group_id 精准清理；无 site_group_id 的表（如 TicketCategory）
  演示环境全清；通过外键间接关联的表（AccountOpeningRequest/CloudComputerUser）
  按演示账号 ID 清理。
- 幂等：清理 + 重建可重复执行。
"""
...
async def clean_demo_business_data() -> dict:
    ...
    清理策略：
    - 有 ``site_group_id`` 的表：按 demo 站点组 ID 清理（含用户演示期间手动创建的）
    - 无 ``site_group_id`` 的表（如 ``TicketCategory``）：演示环境全清
    - 通过外键间接关联的表（``AccountOpeningRequest``/``CloudComputerUser``）：
      按演示账号 ID 清理
    - 保留：演示账号（superadmin/siteadmin/user）和 demo 站点组本身

    清理顺序遵守外键依赖（先删依赖方，再删被依赖方）。

    :returns: {"deleted": {表名: 数量}}
    """
    ...
    async with db_session() as session:
        ...
        if demo_sg is None:
            return {"deleted": deleted, "site_group": None}
        ...
        result = await session.execute(
            delete(PointRecord).where(PointRecord.site_group_id == sg_id)
        )
        deleted["point_record"] = result.rowcount or 0

        result = await session.execute(
            delete(Notification).where(Notification.site_group_id == sg_id)
        )
        deleted["notification"] = result.rowcount or 0

        # ── 2. 积分余额 ──
        result = await session.execute(
            delete(UserPoints).where(UserPoints.site_group_id == sg_id)
        )
        deleted["user_points"] = result.rowcount or 0

        # ── 3. 积分任务 ──
        result = await session.execute(
            delete(PointTask).where(PointTask.site_group_id == sg_id)
        )
        deleted["point_task"] = result.rowcount or 0

        # ── 4. 工单（CASCADE 会自动清理 Comment/Activity/Attachment）──
        result = await session.execute(
            delete(Ticket).where(Ticket.site_group_id == sg_id)
        )
        ...
        # ── 6. 公告 ──
        result = await session.execute(
            delete(Announcement).where(Announcement.site_group_id == sg_id)
        )
        ...
        result = await session.execute(
            delete(Product).where(Product.site_group_id == sg_id)
        )
        deleted["product"] = result.rowcount or 0

        # ── 10. 产品组 ──
        result = await session.execute(
            delete(ProductGroup).where(ProductGroup.site_group_id == sg_id)
        )
        deleted["product_group"] = result.rowcount or 0

        # ── 11. 主机组 ──
        result = await session.execute(
            delete(HostGroup).where(HostGroup.site_group_id == sg_id)
        )
        deleted["host_group"] = result.rowcount or 0

        # ── 12. 主机（最后清理）──
        result = await session.execute(
            delete(Host).where(Host.site_group_id == sg_id)
        )
        deleted["host"] = result.rowcount or 0

    log.info("demo_data_cleaned", deleted=deleted)
    return {"deleted": deleted, "site_group": "demo"}


async def seed_demo_business_data() -> dict:
    """预置演示业务数据。

    前置条件：演示账号与 demo 站点组已存在（由 ``seed_demo_accounts()`` 创建）。
    本函数不创建账号，仅创建业务实体。

    :returns: {"created": {表名: 数量}}
    """
    ...
    async with db_session() as session:
        ...
        if demo_sg is None:
            raise RuntimeError("演示站点组不存在，请先运行 2c2a account demo-seed")
    ...
    log.info("demo_data_seeded", created=created)
