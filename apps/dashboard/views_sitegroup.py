...
from apps.accounts.provider_decorators import superadmin_required, site_group_admin_required
from .models import SiteGroup, SiteGroupHostname, SiteGroupConfig
from .forms_sitegroup import SiteGroupForm, SiteGroupHostnameForm, SiteGroupConfigForm
...
def sitegroup_delete(request, pk):
    ...
    return render(
        request,
        "dashboard/sitegroup_detail.html",
        {
            "sitegroup": sitegroup,
            "active_nav": "sitegroups",
        },
...
def sitegroup_remove_hostname(request, pk, hostname_pk):
    sitegroup = get_object_or_404(SiteGroup, pk=pk)
    hostname = get_object_or_404(
        SiteGroupHostname, pk=hostname_pk, site_group=sitegroup
    )
    if request.method == "POST":
        hostname.delete()
        ...
        if username:
            ...
            try:
                ...
                if user.is_superuser:
                    messages.warning(
                        request, f"用户 {username} 已是超级管理员，无需添加"
                    )
...
@superadmin_required
def sitegroup_config(request, pk):
    """站点组配置覆盖编辑"""
    sitegroup = get_object_or_404(SiteGroup, pk=pk)
    config, _ = SiteGroupConfig.objects.get_or_create(site_group=sitegroup)

    # 获取全局配置用于显示默认值
    from .models import SystemConfig

    global_config = SystemConfig.get_config()

    if request.method == "POST":
        form = SiteGroupConfigForm(request.POST, instance=config)
        if form.is_valid():
            form.save()
            messages.success(request, f"站点组「{sitegroup.name}」配置已更新")
            return redirect("dashboard:sitegroup_config", pk=pk)
    else:
        form = SiteGroupConfigForm(instance=config)

    return render(
        request,
        "dashboard/sitegroup_config.html",
        {
            "sitegroup": sitegroup,
            "form": form,
...
def sitegroup_my_config(request):
    """站点组管理员编辑自己站点组的配置覆盖"""
    site_group = request.site_group
    config, _ = SiteGroupConfig.objects.get_or_create(site_group=site_group)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f"站点组「{site_group.name}」配置已更新")
            return redirect("dashboard:sitegroup_my_config")
    else:
        form = SiteGroupConfigForm(instance=config)
